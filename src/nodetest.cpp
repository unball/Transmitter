...
void receiveRadioTopicMessage(const communication_node::comm_msg& toggle_msg){
...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber strategy_subcriber = n.subscribe("dummy_topic", 1, receiveRadioTopicMessage);
    while (ros::ok())
    {
        ros::spinOnce();
        loop_rate.sleep();
